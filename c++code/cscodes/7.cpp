...
int main () {
	...
	if (a == 1) {
		...
		num[a]--;
		sc (num);
		system ("yx-2\\PCL2_CE\\PCL2_CE.exe");
	} else if (a == 3) {
		vector<int> num = sr ();
		if (num[a] == 0) {
			cout << "您的次数已经用完，无法启动！！！\n";
		...
		num[a]--;
		sc (num);
		system ("yx-2\\ZJ-1\\PlantsVsZombiesRH.exe");
	} else if (a == 4) {
		vector<int> num = sr ();
		if (num[a] == 0) {
			cout << "您的次数已经用完，无法启动！！！\n";
		...
		num[a]--;
		sc (num);
		system ("yx-2\\ZJ-2\\PlantsVsZombiesRH.exe");
	} else {
		cout << "cnm,没有这个选项";
	}
}
