//
// Created by wywhpsmaq on 2026/6/19.
//
...
// 1. 矩阵转 JSON（手动拼接）
// ------------------------------------------------------------
inline std::string matrixToJson(const std::vector<std::vector<int>>& matrix) {
...
// ------------------------------------------------------------
inline std::string httpPost(const std::string& host, const int port, const std::string& path, const std::string& body) {
	WSADATA wsaData;
	...
	const std::string request = requestStream.str();
...
// ------------------------------------------------------------
inline bool extractBody(const std::string& httpResponse, std::string& body) {
	...
	const std::string headers = httpResponse.substr(0, headerEnd);
	std::string rawBody = httpResponse.substr(headerEnd + 4);

	// 检测 Content-Length
	const std::regex contentLengthRegex("Content-Length: (\\d+)", std::regex::icase);
	long long contentLength = -1;
	if (std::smatch match; std::regex_search(headers, match, contentLengthRegex) && match.size() > 1) {
		contentLength = std::stoll(match[1]);
	}

	// 检测 chunked

	if (const bool isChunked = headers.find("Transfer-Encoding: chunked") != std::string::npos; !isChunked && contentLength >= 0) {
		if (rawBody.size() >= static_cast<size_t>(contentLength)) {
			body = rawBody.substr(0, static_cast<size_t>(contentLength));
			return true;
		} else {
		...
		while (pos < rawBody.size()) {
			...
			std::erase_if(chunkSizeStr, ::isspace);
...
// 4. 核心函数：传入矩阵，生成并保存图片
// ------------------------------------------------------------
inline bool generateImageFromMatrix(const std::vector<std::vector<int>>& matrix, const std::string& filename) {
	if (matrix.size() != 50) {
	...
	const std::string jsonBody = matrixToJson(matrix);
	...
	// 可选：打印响应头（调试用）
	if (const size_t headerEnd = response.find("\r\n\r\n"); headerEnd != std::string::npos) {
		// std::cout << "=== 响应头部 ===\n" << response.substr(0, headerEnd) << "\n================" << std::endl;
	}

	std::string imageData;
	const bool success = extractBody(response, imageData);
	if (imageData.empty()) {
