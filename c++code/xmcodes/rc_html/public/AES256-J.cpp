...
void hexStringToState (const std::string &hexStr, State &state) {
	for (int col = 0; col < 4; col++) {
		for (int row = 0; row < 4; row++) {
			std::string byteStr = hexStr.substr (col * 8 + row * 2, 2);
...
int main () {
	...
	std::ifstream ciphertextFile ("jm-in.txt");
	if (!ciphertextFile.is_open ()) {
		//std::cerr << "无法打开密文文件!" << std::endl;
		return 1;
	}
	std::string ciphertextStr;
	ciphertextFile >> ciphertextStr;
	ciphertextFile.close ();
	if (ciphertextStr.length () != 32) {
		//std::cerr << "密文长度错误! 必须是32个十六进制字符." << std::endl;
		return 1;
	}
	State state;
	aes_2::hexStringToState (ciphertextStr, state);
	...
	for (int round = 13; round >= 1; round--) {
		...
		//std::cout << "round " << round << " decryption completed" << std::endl;
	}
	aes_2::invShiftRows (state);
	aes_2::invSubBytes (state);
	aes_2::addRoundKey (state, roundKeys);
	std::string plaintext = aes_2::stateToHexString (state);
	std::ofstream outFile ("jm-out.txt");
	if (outFile.is_open ()) {
		outFile << plaintext;
		outFile.close ();
		//std::cout << "解密成功，明文已写入 jm-out.txt" << std::endl;
	} else {
